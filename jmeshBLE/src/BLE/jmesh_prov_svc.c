...
struct gattm_att_desc const jmesh_prov_svc_att_db[JMESHS_IDX_PROV_NB] = {
    ...
    [JMESHS_IDX_MESH_NETWORK_TRANSMIT_IN_VAL]     = {{0x01,0x2b}, PERM(WP, NO_AUTH)|PERM(WRITE_REQ, ENABLE), NETWORK_TRANSMIT_LENGTH, PERM(RI, ENABLE)},
		
    [JMESHS_IDX_MESH_NETWORK_TRANSMIT_OUT_CHAR]    = {{ATT_DECL_CHARACTERISTIC&0xff,ATT_DECL_CHARACTERISTIC>>8}, PERM(RD, ENABLE), 0, 0},
    [JMESHS_IDX_MESH_NETWORK_TRANSMIT_OUT_VAL]     = {{0x00,0x2b}, PERM(RD, ENABLE)|PERM(NTF,ENABLE), NETWORK_TRANSMIT_LENGTH, PERM(RI, ENABLE)},
