...
static void link_hold_timeout_handler()
{
    prov_reset_progress();
...
static uint16_t gatt_provisioning_capabilities_handler(uint8_t* dat, uint8_t* dat_out)
{
    ...
    jmesh_proxy_send(jmesh_gatt_get_interface(jmesh_gatt_get_by_id(provisioning_connect)),JMESH_PROXY_TYPE_PROVISION,pdu);
...
/*
void prov_test_connected(unsigned char* mac){
    jmesh_connected(mac,1);
    print_info("connected");
}
static os_timer_event_t prov_test_timer;
static int server_len,client_len;
void prov_test_server_recv(unsigned char* data);
void prov_test_client_recv(unsigned char* data)
{
    static unsigned char buff[100];
    int len;

    print_buffer_info(server_len,data,"client read data:");
    len=gatt_provisioning_client_handlers(data,buff);
    client_len=len;
    print_buffer_info(len,buff,"client return data:");
    jmesh_proxy_send(jmesh_connect_get_interface(jmesh_connect_get_by_id(provisioning_connect)),JMESH_PROXY_TYPE_PROVISION,len,buff);
    os_timer_event_set(&prov_test_timer,500,(os_timer_event_caller_t)prov_test_server_recv,buff);
}
...
void prov_test_server_recv(unsigned char* data){
    ...
    print_buffer_info(client_len,data,"server read data:");
    len=gatt_provisioning_server_handlers(data,buff);
    server_len=len;
    print_buffer_info(len,buff,"server return data:");
    jmesh_proxy_send(jmesh_connect_get_interface(jmesh_connect_get_by_id(provisioning_connect)),JMESH_PROXY_TYPE_PROVISION,len,buff);
    os_timer_event_set(&prov_test_timer,500,(os_timer_event_caller_t)prov_test_client_recv,buff);
}
void provisioning_test(void)
{
    static unsigned char mac[6]={0,1,2,3,4,5};

    jmesh_provisioning_device(mac);
    os_timer_event_set(&prov_test_timer,500,(os_timer_event_caller_t)prov_test_connected,mac);
}
*/
...
void jmesh_provisioning_connected(unsigned char* mac,unsigned short connect_id){
    ...
    if(0==memcmp(provisioning_mac,mac,6)){
        ...
        os_timer_event_set(&provisioning_timer, 1000, prov_result_time_flow, NULL);
