...
void qspi_init(uint16_t clk_div,uint8_t rxsample_dly)
{
    ...
    QSPI_REG->SPI_CTRLR0 = FIELD_BUILD(SSI_WAIT_CYCLES,QSPI_MULTI_READ_WAIT_CYCLES) | FIELD_BUILD(SSI_INST_L, Instruction_Length_8_bits)|
             FIELD_BUILD(SSI_ADDR_L,Addr_Width_24_bits)|FIELD_BUILD(SSI_TRANS_TYPE, Both_Standard_SPI_Mode);
    QSPI_REG->SER = 0;
...
static void qspi_multi_read(uint8_t cs_sel_mask,uint8_t cmd,uint32_t addr,uint8_t *data,uint32_t length,uint8_t spi_mode)
{
    uint32_t length_in_word = length / sizeof(uint32_t);
    uint8_t remainder = length % sizeof(uint32_t);
    if(length_in_word)
    {
        QSPI_REG->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,spi_mode) | FIELD_BUILD(SSI_DFS,DFS_32_32_bits) | FIELD_BUILD(SSI_TMOD, EEPROM_Read) |
            FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
    ...
    if(remainder)
    {
        QSPI_REG->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,spi_mode) | FIELD_BUILD(SSI_DFS,DFS_32_8_bits) | FIELD_BUILD(SSI_TMOD, EEPROM_Read) |
            FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
        QSPI_REG->CTRLR1 = remainder - 1;
        QSPI_REG->DMACR = 0;
        QSPI_REG->SSIENR = SSI_Enabled;
        QSPI_REG->DR = cmd;
        QSPI_REG->DR = addr + length_in_word*sizeof(uint32_t);
        QSPI_REG->SER = cs_sel_mask;
        while(FIELD_RD(QSPI_REG,SR, SSI_TFE)!=Transmit_FIFO_Empty);
        while(FIELD_RD(QSPI_REG,SR, SSI_BUSY)==SSI_Busy);
        uint8_t i;
        uint8_t *remainder_ptr = &data[length_in_word*sizeof(uint32_t)];
...
{
    qspi_multi_read(QSPI_SLAVE_INDEX,cmd,addr,data,length,Dual_SPI_Format);
}

void qspi_quad_read(uint8_t cmd,uint32_t addr,uint8_t *data,uint32_t length)
{
    qspi_multi_read(QSPI_SLAVE_INDEX,cmd,addr,data,length,Quad_SPI_Format);
}

void qspi_config_for_cache(uint16_t cache_line_size,uint8_t spi_mode)
{
    QSPI_REG->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,spi_mode) | FIELD_BUILD(SSI_DFS,DFS_32_32_bits) | FIELD_BUILD(SSI_TMOD, Receive_Only) |
        FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
    QSPI_REG->CTRLR1 = cache_line_size/sizeof(uint32_t) - 1;
    QSPI_REG->SER = QSPI_SLAVE_INDEX;
    QSPI_REG->SSIENR = SSI_Enabled;
}

uint32_t qspi_dr_reg_addr_for_cache()
{
    return (uint32_t)&QSPI_REG->DR;
}

void qspi_disable_for_cache()
{
    while(FIELD_RD(QSPI_REG, SR,SSI_BUSY));
    QSPI_REG->SSIENR = SSI_Disabled;
    QSPI_REG->SER = 0;
}
