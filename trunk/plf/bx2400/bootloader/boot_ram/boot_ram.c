...
#define BX2400_A_CRC ((crc_func_t)0x1b1)
#define BX2400_B_CRC ((crc_func_t)0x18d)
...
{
    if(image_header[0].feature.valid== 0 && image_header[1].feature.valid == 0)
    {
        return image_header[0].image_id >  image_header[1].image_id ? &image_header[0] : &image_header[1];
    }else if(image_header[0].feature.valid == 0)
    {
        return &image_header[0];
    }else if(image_header[1].feature.valid == 0)
    {
...
//    boot_ram_awo_reset();
    ...
    image_header_t *image = image_select();
    if(image == NULL)
    {
        system_reset();
    }
    ...
    if(image->feature.dual_quad == Quad_SPI_Format)
    {
        ...
        flash_quad_read(image_offset, image->image_ram_size, (uint8_t *)RAM_BASE);
    }else if(image->feature.dual_quad == Dual_SPI_Format)
    {
        sysc_cpu_qspi_en_setf(0x3);
        flash_dual_read(image_offset, image->image_ram_size, (uint8_t *)RAM_BASE);
    }else if(image->feature.dual_quad == Standard_SPI_Format)
    {
        sysc_cpu_qspi_en_setf(0x3);
    ...
    crc_func_t crc16ccitt = (hwp_sysc_awo->rf_reg_rd.val & 0x1f) >=  0x14 ? BX2400_B_CRC : BX2400_A_CRC;
    ...
    (*reset_handler)(flash_offset,boot_ram_header.nvds_base,
        image->feature.dual_quad,BOOTINIT_FLAG);        //0x3399 indicate bootram has runned.
}
