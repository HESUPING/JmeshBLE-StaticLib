...
#define PD_SRAM_NULL        0

#define PD_POWER_ON			    0
#define PD_POWER_DOWN			1


///define for clk_gate @ awo
#define CLKG_HBUS_DIV_SET		        CO_BIT(0)
#define CLKG_HBUS_DIV_CLR		        CO_BIT(1)
#define CLKG_BLE_DIV_SET		        CO_BIT(2)
#define CLKG_BLE_DIV_CLR		        CO_BIT(3)
#define CLKG_BLE_LP_SET		            CO_BIT(4)
#define CLKG_BLE_LP_CLR		            CO_BIT(5)
#define CLKG_RTC_SET		            CO_BIT(6)
#define CLKG_RTC_CLR		            CO_BIT(7)
#define CLKG_TOUCH_SET		            CO_BIT(8)
#define CLKG_TOUCH_CLR		            CO_BIT(9)

///register in sysc_cpu
#define CLKG_MDM_SET                    CO_BIT(20)
#define CLKG_MDM_CLR                    CO_BIT(21)
#define CLKG_BLE_SET                    CO_BIT(22)
#define CLKG_BLE_CLR                    CO_BIT(23)


///define for sft_rst signal
#define SRST_ALL_SET            CO_BIT(0)
#define SRST_ALL_CLR		    CO_BIT(1)
#define SRST_RTC_SET		    CO_BIT(2)
#define SRST_RTC_CLR		    CO_BIT(3)
#define SRST_BLE_N_SET          CO_BIT(4)
#define SRST_BLE_N_CLR          CO_BIT(5)
#define SRST_TOUCH_N_SET        CO_BIT(6)
#define SRST_TOUCH_N_CLR        CO_BIT(7)
#define SRST_BLE_LP_SET        CO_BIT(8)
#define SRST_BLE_LP_CLR        CO_BIT(9)



///define for intr_ctrl @ awo
#define EXT_INT_0			0
#define EXT_INT_1			1
#define EXT_INT_2			2
#define EXT_INT_3			3
...
///define for clk_ctrl1 @ awo
#define PARAM_UP_DIV_BLE_SW	CO_BIT(12)
#define PARAM_UP_PBUS       CO_BIT(8)



///define for ext_rawinrp: trigger type @ awo
#define TRIG_HIGH_LEVEL		0
#define TRIG_LOW_LEVEL		1
#define TRIG_POS_EDGE		2
#define TRIG_NEG_EDGE		3
...
#define PLL_STABLE_BIT      5

#define IO_IE_SET(mask) 
#define IO_IE_CLR(mask) 
 ...
 * @brief close pll.
 */
void awo_close_pll(void);
/**
 * @brief open pll and set pll frequency. When this function return pll may not ready. We should use awo_wait_pll_stable() to wait pll stable.
 * @param freq: xx Mhz. if we want to set pll to 96Mhz freq = 96.
 */
void awo_open_pll(uint8_t freq);
/**
 ...
 */
void awo_clkgate_en(uint32_t map);

void awo_clkgate_mdm(uint8_t en);
void awo_clkgate_ble(uint8_t en);
 ...
 */
void awo_pmu_io_retention(uint32_t value, uint32_t enable);
...
void awo_rc32m_pwr(uint8_t onoff);
 ...
 */
void awo_ram_param_write(uint8_t rme, uint8_t rm, uint8_t wa, uint8_t ra, uint8_t wpulse, uint8_t sram_volt);
/**
 * @brief read ram parameter.
 */
void awo_ram_param_read(uint8_t *rme, uint8_t *rm, uint8_t *wa, uint8_t *ra, uint8_t *wpulse, uint8_t *sram_volt);
/**
 ...
 */
void awo_touch_start(uint16_t interval, uint8_t num, uint8_t dly, uint32_t thd, uint8_t drop_num, uint8_t intr);
