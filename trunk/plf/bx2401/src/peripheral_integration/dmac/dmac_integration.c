...
static periph_intr_env_t dmac_intr_env;

static void dmac_intr_op(app_dmac_inst_t *inst,intr_operation_t op,void (*isr)(void *))
{
    if(op == INTR_ENABLE)
    {
        dmac_intr_env.inst_ptr = inst;
        dmac_intr_env.isr = isr;
...
periph_universal_func_set_t dmac_universal_func =
{
    ...
    .clk_src_cfg_func = (clk_src_cfg_func_t)dmac_dummy,
    .clk_gate_func = (clk_gate_func_t)dmac_clk_gate,
    .pin_cfg_func = (clk_src_cfg_func_t)dmac_dummy,
    .sys_stat_func = (sys_stat_func_t)dmac_sys_stat,
};

void DMAC_IRQHandler()
{
    dmac_intr_env.isr(dmac_intr_env.inst_ptr);
}
