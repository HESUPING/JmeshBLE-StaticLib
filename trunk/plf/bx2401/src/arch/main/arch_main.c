...
static void uart0_read(uint8_t * bufptr, uint32_t size, void(* callback)(void *, uint8_t), void * dummy)
{
    app_uart_read(&uart0.inst,bufptr,size,callback, dummy);
}

static void uart0_write(uint8_t * bufptr, uint32_t size, void(* callback)(void *, uint8_t), void * dummy)
{
    app_uart_write(&uart0.inst,bufptr,size,callback, dummy);
}

static void uart0_flow_on()
{
    app_uart_flow_on(&uart0.inst);
}

static bool uart0_flow_off()
{
...
//    intc_init();
    ...
    #if (PLF_NVDS)
    nvds_init((uint8_t*)(current_image_base + nvds_base),0);
    #endif
...
//    rc_calib_end();
    ...
    while(1);
    #else
//    rtc_rtc_ccr_pack(0,1,0,0);
    debug_init();
    sysc_ble_ble_freq0_setf(32); 
    sysc_awo_clk_ctrl1_set(sysc_awo_clk_ctrl1_get() | PARAM_UP_DIV_BLE_SW);
