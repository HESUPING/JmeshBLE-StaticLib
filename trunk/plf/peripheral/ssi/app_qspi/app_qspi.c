...
void app_qspi_init(periph_inst_handle_t hdl)
{
    ...
    if(inst->param.mode != Standard_SPI_Format)
    {
    ...
    inst->reg->SPI_CTRLR0 = FIELD_BUILD(SSI_WAIT_CYCLES,QSPI_MULTI_READ_WAIT_CYCLES) | FIELD_BUILD(SSI_INST_L, Instruction_Length_8_bits)|
...
void app_qspi_multi_read(periph_inst_handle_t hdl,uint8_t cs_sel_mask,uint8_t cmd,uint32_t addr,uint8_t *data,uint32_t length,uint8_t spi_mode)
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    reg_ssi_t *reg = inst->reg;
    BX_ASSERT(length < 0x40000);
    ...
    if(length_in_word)
    {
        reg->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,spi_mode) | FIELD_BUILD(SSI_DFS,DFS_32_32_bits) | FIELD_BUILD(SSI_TMOD, Receive_Only) |
            FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
    ...
    if(remainder)
    {
        reg->CTRLR0 = FIELD_BUILD(SSI_SPI_FRF,spi_mode) | FIELD_BUILD(SSI_DFS,DFS_32_8_bits) | FIELD_BUILD(SSI_TMOD, Receive_Only) |
            FIELD_BUILD(SSI_SCPOL,Inactive_Low)|FIELD_BUILD(SSI_SCPH, SCLK_Toggle_In_Middle) | FIELD_BUILD(SSI_FRF, Motorola_SPI);
        reg->CTRLR1 = remainder - 1;
        reg->DMACR = 0;
        reg->SSIENR = SSI_Enabled;
        reg->DR = cmd;
        reg->DR = addr + length_in_word*sizeof(uint32_t);
        reg->SER = cs_sel_mask;
...
void app_qspi_config_for_cache(periph_inst_handle_t hdl,uint8_t cs_sel_mask,uint16_t cache_line_size,uint8_t spi_mode)
{
    app_qspi_inst_t *inst = CONTAINER_OF(hdl, app_qspi_inst_t, inst);
    reg_ssi_t *reg = inst->reg;
    qspi_universal_func.clk_gate_func(inst,SET_CLK);
