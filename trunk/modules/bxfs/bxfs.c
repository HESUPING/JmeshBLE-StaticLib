...
void bxfs_init(uint32_t base,uint16_t section_num,uint32_t flash_dual_quad)
{
    BX_ASSERT(((uint32_t)base&0xfff)==0);
    BX_ASSERT(section_num>=3);
    if(flash_dual_quad == Quad_SPI_Format)
    {
        bxfs_env.read = flash_quad_read;
    }else if(flash_dual_quad == Dual_SPI_Format)
    {
        bxfs_env.read = flash_dual_read;
    }else{
        bxfs_env.read = flash_read;
    }
