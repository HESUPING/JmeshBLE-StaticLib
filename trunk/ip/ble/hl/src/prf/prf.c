...
static const struct prf_task_cbs * prf_itf_get(uint16_t task_id)
{
    ...
    switch(KE_TYPE_GET(task_id))
    {
        ...
        #if (BLE_JMESHS_PROV_SERVER)
				case TASK_ID_JMESHS_PROV:
            prf_cbs = jmeshs_prov_prf_itf_get();
            break;
				#endif	
        ...
        #if (BLE_JMESHS_APP_SERVER)
				case TASK_ID_JMESHS_APP:
            prf_cbs = jmeshs_app_prf_itf_get();
            break;
				#endif	
