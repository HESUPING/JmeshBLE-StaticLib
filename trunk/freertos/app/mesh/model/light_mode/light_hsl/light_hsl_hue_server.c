...
void light_hsl_hue_msg_complete(access_pdu_tx_t *p_pdu,uint8_t type)
{


}
void handle_light_hsl_hue_six_sec_timer(TimerHandle_t thandle)
{
	generic_valid_field_check_t value;
	inst_parma_t *inst_parma = (inst_parma_t *)pvTimerGetTimerID(thandle);
	light_hsl_hue_server_t *server = (light_hsl_hue_server_t *)inst_parma->inst;
	value = generic_msg_queue_pop(&server->hsl_hue_queue);
	xTimerDelete(value.Timer,0);	
}

static void handler_light_hsl_hue_transition_timer(TimerHandle_t thandle)
{	
	light_hsl_hue_delay_parma_t  *timer_parma = (light_hsl_hue_delay_parma_t *)pvTimerGetTimerID(thandle);
	
	light_hsl_hue_server_t *server = (light_hsl_hue_server_t *)timer_parma->inst;

	server->light_hsl_server->msg_format.present_hsl_hue = timer_parma->target_hsl_hue;
...
static void handler_light_hsl_hue_delay_timer(TimerHandle_t thandle)
{
	uint32_t ms;
	...
	server->light_hsl_server->msg_format.target_hsl_hue = timer_parma->target_hsl_hue;
	...
	server->delay_timer.trans_timer = xTimerCreate("Timer",pdMS_TO_TICKS(ms),pdTRUE,
							(void *)timer_parma,handler_light_hsl_hue_transition_timer);	//transtion timer
	xTimerStart(server->delay_timer.trans_timer,0);
}

void light_hsl_hue_status(light_hsl_hue_server_t *server,uint16_t dst_addr)
{
	uint32_t tick;
	...
	if(server->server_state == GENERIC_TRANS_PROCESS)
	{
		...
		access_message_tx.cb = light_hsl_hue_msg_complete;
...
void light_hsl_hue_set_handle(mesh_elmt_t *elmt,model_base_t *model,access_pdu_rx_t *pdu)
{
	...
	light_hsl_hue_server_t *server = GET_SERVER_MODEL_PTR(light_hsl_hue_server_t,model);
	server->hsl_hue_queue.inst_parma.inst = (void *)server;
	
	if(payload_size == sizeof(msg_light_hsl_hue_set_t))
	{
			
		msg_light_hsl_hue_set_t *p_pdu = (msg_light_hsl_hue_set_t *)(pdu->access + 2);
		msg_field.tid = p_pdu->tid;
		result = generic_msg_queue_search(&server->hsl_hue_queue,msg_field);
		if(result)
		{
		...
		result = generic_msg_queue_push(&server->hsl_hue_queue,msg_field);
		
		server->hsl_hue_queue.date[result].Timer = xTimerCreate("Timer",
				pdMS_TO_TICKS(6000),pdFALSE,(void *)server->hsl_hue_queue.inst,handle_light_hsl_hue_six_sec_timer); //6s
										
		if(server->hsl_hue_queue.date[result].Timer != NULL)
				xTimerStart(server->hsl_hue_queue.date[result].Timer,0);	
		{//this block code is used to transition state	
		result =  generic_timer_free_get(server->delay_timer.mask);
		   
		server->delay_timer.delay_parma[result].inst = server->hsl_hue_queue.inst;
		...
		server->delay_timer.Timer[result] = xTimerCreate("Timer",
				pdMS_TO_TICKS(p_pdu->delay * 5),pdFALSE,(void *)&server->delay_timer.delay_parma[result],handler_light_hsl_hue_delay_timer);		//5ms * p_pdu->delay
		if(server->delay_timer.Timer[result] != NULL)
				xTimerStart(server->delay_timer.Timer[result],0);
			}
		
		}
	else if(payload_size == sizeof(msg_light_hsl_lite_hue_set_t))
	{
		msg_light_hsl_lite_hue_set_t *p_pdu = (msg_light_hsl_lite_hue_set_t *)(pdu->access + 2);
		msg_field.tid = p_pdu->tid;
		result = generic_msg_queue_search(&server->hsl_hue_queue,msg_field);
		if(result)
		{
		...
		if(server->server_state == GENERIC_TRANS_PROCESS)
			...
			if(result)
			{
				...
				pdMS_TO_TICKS(ms),pdFALSE,(void *)&server->delay_timer.delay_parma[index],handler_light_hsl_hue_transition_timer);		//5ms * p_pdu->delay
			}
			else
			{
				server->delay_timer.delay_parma[index].timer_index = 0xff;
				handler_light_hsl_hue_transition_timer((void *)&server->delay_timer.delay_parma[index]);
			}  
...
{
	light_hsl_hue_set_handle(elmt,model,pdu);
}
